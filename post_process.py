...
class Param(object):
	...
	# params for acrt
	def __init__(self, sgm_P1=0.01, sgm_P2=1, sgm_Q1=4.5, sgm_Q2=9, sgm_V=2.75, sgm_D=0.13, blur_sigma=1.7, blur_thre=2):
	# # params for fst
...
if __name__ == "__main__":
	...
	# acrt
	path = "../result_acrt/"+name+"/"
	left_disp_path = path+"raw_disp.png" #path+"_l/raw_disp.png"
	# right_disp_path = path+"raw_disp_right.png" #path+"/raw_disp.png"
	left_mcv_path =  path+"im0.kzr"
	# right_mcv_path = path+"im0_right.kzr" #path+"/im0.acrt"
	# path = path+"/post"
	path = "../result_acrt/"+name+"/post_2"
	if not os.path.exists(path):
		os.makedirs(path)

	# # fst on server
	# path = "../result_10M/"+name
	# pathr = "../result_10M_r/"+name
	# left_disp_path = path+"/raw_disp.png"
	# right_disp_path = pathr+"/raw_disp.png"
	# left_mcv_path =  path+"/im0.acrt"
	# right_mcv_path = pathr+"/im0.acrt" 
	# path = "../result_10M_post/"+name
	# if not os.path.exists(path):
	# 	os.makedirs(path)
	...
	subpixel_enhancement(left_disp_after_path, disp_subpixel_path, left_mcv_after_path, dmax )
	print("filter....."+str(datetime.datetime.now()))
	median_filter(disp_subpixel_path, disp_median_path)
	bilateral_filter(disp_median_path, disp_bilateral_path, sigma_color=blur_thre, sigma_space=blur_sigma)
